...
from src.utils import DATASET_DEFAULTS, VIS_SEED, infer_variant, load_val_samples


def pca_feature_map(feat: torch.Tensor, n_components: int = 3,
                    pca_model=None) -> tuple[np.ndarray, None]:
    """Project a spatial feature map to n_components using GPU-accelerated PyTorch PCA.
    ...
    Args:
        feat: (B, C, H, W) tensor.
        pca_model: Not used (re-fit is fast and on-device).
    Returns:
    ...
    # Project data onto the first n_components eigenvectors
    proj = torch.matmul(X_centered, eigenvectors[:, :n_components])
    proj = proj.reshape(B, H, W, n_components)
    ...
    return proj.cpu().float().numpy(), None
